      ...
      <nav className="flex flex-wrap gap-x-8 gap-y-3 border-b border-border mb-10">
        {tabs.map((t) => (
          <NavLink key={t.to} to={t.to} end={t.end}
            className={({ isActive }) =>
              `text-xs uppercase tracking-luxe pb-3 -mb-px border-b-2 ${isActive ? "border-accent text-accent" : "border-transparent text-muted-foreground hover:text-foreground"}`
            }>
            {t.label}
          </NavLink>
        ))}
