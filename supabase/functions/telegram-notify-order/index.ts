...
function fmtBRL(n: number, currency = 'BRL') {
  try {
...
function buildMessage(payload: any): string {
  ...
  const itemsTxt = items.slice(0, 10).map((it: any) =>
    `• ${it.quantity ?? it.qty ?? 1}x ${it.title ?? it.name ?? it.handle ?? 'item'}`
  ).join('\n') || '—';
  ...
  if (payload.event === 'created') {
    return `🛒 <b>Novo pedido</b>\n` +
      ...
      (o.checkout_url ? `\n<a href="${o.checkout_url}">Checkout</a>` : '');
  }
  if (payload.event === 'status_changed') {
    return `🔄 <b>Status do pedido alterado</b>\n` +
      `ID: <code>${o.id}</code>\n` +
      `<b>${payload.old_status}</b> → <b>${payload.new_status}</b>\n` +
      `Total: ${fmtBRL(Number(o.total ?? 0), o.currency)}`;
  }
  return `Pedido ${o.id}: evento ${payload.event}`;
}
